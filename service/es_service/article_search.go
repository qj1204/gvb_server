...
func CommonDetail(id string) (article models.ArticleModel, err error) {
	...
	article.DiggCount = article.DiggCount + redis_service.NewArticleDiggCount().Get(id)
	article.LookCount = article.LookCount + redis_service.NewArticleLookCount().Get(id)
	article.CommentCount = article.CommentCount + redis_service.NewArticleCommentCount().Get(id)
	return
}
