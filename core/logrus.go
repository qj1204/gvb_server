...
func InitLogger() *logrus.Logger {
	...
	myLog.SetReportCaller(global.Config.Logger.ShowLine) // 开启返回函数名和行号
...
func InitFileLogger(myLog *logrus.Logger) {
	...
	//创建文件
	filename := fmt.Sprintf("%s/%s/%s.log", global.Config.Logger.Director, fileDate, global.Config.Logger.Prefix)
	...
	fileHook := FileDateHook{file, global.Config.Logger.Director, fileDate, global.Config.Logger.Prefix}
