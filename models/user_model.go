...
type UserModel struct {
	...
	IP         string           `gorm:"size:20;comment:'IP'" json:"ip,select(c)"`                         // IP
	Role       ctype.Role       `gorm:"size:4;default:1;comment:'角色'" json:"role,select(info)"`           // 角色
	SignStatus ctype.SignStatus `gorm:"type=smallint(6)';comment:'注册来源'" json:"sign_status,select(info)"` // 注册来源
