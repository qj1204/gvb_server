...
type CommentModel struct {
	...
	// --------子评论 belongs to 父评论--------
	ParentCommentID    *uint         `gorm:"comment:'父评论ID'" json:"parent_comment_id,select(c)"` // 父评论ID（可以为空）
	...
	CommentCount int    `gorm:"size:8;default:0;comment:'评论量'" json:"comment_count,select(c)"` // 子评论量
